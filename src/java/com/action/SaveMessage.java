...
public class SaveMessage extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String date = /* "2018-" + request.getParameter("month") + "-" + request.getParameter("day"); */ request.getParameter("date");
        //String[] str = date.split("/");
        //String newDate = str[2] + "-" + str[0] + "-" + str[1];
        db.open();
        if (db.duplicate(date) > 0) {
            db.close();
            db.open();
            db.update(message, date);
            db.close();
        } else {
            db.close();
            db.open();
            db.saveMessage(date, message);
            db.close();
